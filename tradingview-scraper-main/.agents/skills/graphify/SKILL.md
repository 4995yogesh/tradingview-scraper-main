...
name: graphify
description: >
  Repository Architecture Awareness Mode. Uses Graphify analysis to navigate and modify
  the repository with informed structural context. Ensures high-level awareness of
  subsystems, God Nodes, and community cohesion.
---

Use the generated graph analysis as your architectural source of truth. Locate data in `graphify-out/`.
...
- **Inferred Edges**: If a connection is marked as `[INFERRED]`, verify it manually with `grep_search` or `view_file` (with StartLine/EndLine) before assuming the relationship exists.
